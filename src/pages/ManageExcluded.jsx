...
import ExcludedFormModal from './exclude/ExcludedFormModal';
...
function ManageExcluded() {
    ...
    // State for Add/Edit Excluded Item Modal
    ...
    const fetchExcludedItems = useCallback(async () => {
        try {
            const response = await axios.get(`${BACKEND_URL}/api/excluded`, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
    ...
    useEffect(() => {
        ...
        fetchLanguages();
        fetchExcludedItems();
    }, [isAuthenticated, user, isAuthReady, navigate, fetchExcludedItems, fetchLanguages]);
    ...
    const openFormModal = useCallback(async (item = null) => {
        ...
        if (item) {
            setIsEdit(true);
            try {
                // Fetch detail item dengan terjemahan
                const response = await axios.get(`${BACKEND_URL}/api/excluded/${item._id}`, {
                    headers: { Authorization: `Bearer ${authToken}` },
                });
            ...
            const initialTranslations = languages.map(lang => ({
                language: { _id: lang._id, name: lang.name, code: lang.code },
                name: '',
                description: '',
            }));
    ...
    const handleDelete = useCallback(async () => {
        ...
        try {
            await axios.delete(`${BACKEND_URL}/api/excluded/${itemToDelete._id}`, {
                headers: { Authorization: `Bearer ${authToken}` },
            });
            setExcludedItems(excludedItems.filter((item) => item._id !== itemToDelete._id));
            closeConfirmModal();
            setError('');
        } catch (err) {
            handleApiError(err, 'Failed to delete excluded item.');
            closeConfirmModal();
        }
    }, [itemToDelete, BACKEND_URL, authToken, excludedItems, closeConfirmModal, handleApiError]);
                ...
                <div className="pt-20 max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="bg-white rounded-lg shadow-sm p-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-semibold text-havanaGray">Manage Excluded Items</h3>
                            <button
...
<tbody className="bg-white divide-y divide-gray-200">
  {excludedItems.length === 0 && !error ? (
    <tr>
      <td
        colSpan="3"
        className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center"
      >
        No excluded items found.
      </td>
    </tr>
  ) : (
    excludedItems.map((item) => {
      // cari translation english dulu
      const enTranslation = item.translations?.find(
        (t) => t.language?.code === "en"
      );

      // kalau ga ada english, fallback ke index 0 (indo)
      const translation = enTranslation || item.translations?.[0];

      const displayName = item.name || translation?.name || "-";
        ...
        <tr key={item._id}>
          ...
          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <button
              ...
              aria-label={`Edit excluded item ${displayName}`}
            >
            ...
            <button
              ...
              aria-label={`Delete excluded item ${displayName}`}
            >
              <BiTrash className="text-lg" />
                ...
                <ExcludedFormModal
                    ...
                    authToken={authToken}
                    fetchExcludedItems={fetchExcludedItems}
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p className="text-gray-700 mb-6">
                                        Apakah Anda yakin ingin menghapus item yang dikecualikan <strong>{itemToDelete?.name}</strong>?
