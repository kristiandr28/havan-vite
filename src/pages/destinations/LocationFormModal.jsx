...
function LocationFormModal({
  isOpen, onClose, isEdit, currentLocation, setCurrentLocation, languages,
  BACKEND_URL, authToken, fetchLocations, handleApiError
}) {
  ...
  const handleTranslationChange = useCallback((id, field, value) => {
    const newTranslations = (currentLocation.translations || []).map(t =>
      t.id === id ? { ...t, [field]: value } : t
    );
    setCurrentLocation(prev => ({ ...prev, translations: newTranslations }));
  ...
  const addTranslation = useCallback(() => {
    ...
    setCurrentLocation(prev => ({
      ...prev,
      translations: [...(prev.translations || []), { id: newId, language: '', name: '' }]
    }));
  }, [setCurrentLocation]);

  const removeTranslation = useCallback((id) => {
    const newTranslations = (currentLocation.translations || []).filter(t => t.id !== id);
    setCurrentLocation(prev => ({ ...prev, translations: newTranslations }));
  }, [currentLocation, setCurrentLocation]);

  const validateForm = useCallback(() => {
    const errors = {};
    if (!currentLocation?.translations?.length) {
      errors.translations = 'At least one translation is required.';
    } else {
      currentLocation.translations.forEach((t, index) => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    formData.append('translations', JSON.stringify(currentLocation.translations.map(({ id, ...rest }) => rest)));

    const endpoint = isEdit ? `${BACKEND_URL}/api/locations/${currentLocation._id}` : `${BACKEND_URL}/api/locations`;
    ...
    try {
      ...
      handleApiError(err, `Failed to ${isEdit ? 'update' : 'create'} location.`);
    }
  }, [BACKEND_URL, authToken, isEdit, currentLocation, fetchLocations, onClose, handleApiError, validateForm]);
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-havanaBlue mb-4">{isEdit ? 'Edit Location' : 'Add Location'}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
              ...
              <div className="max-h-48 overflow-y-auto border rounded-md p-2 custom-scrollbar">
                {(currentLocation?.translations || []).map((t, index) => (
