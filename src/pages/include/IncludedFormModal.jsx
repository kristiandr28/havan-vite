...
function IncludedFormModal({
  isOpen, onClose, isEdit, currentItem, setCurrentItem, languages,
  BACKEND_URL, authToken, fetchIncludedItems, handleApiError
}) {
  ...
  const handleTranslationChange = useCallback((langId, field, value) => {
    setCurrentItem(prev => {
      const updatedTranslations = prev.translations.map(t =>
        t.language === langId ? { ...t, [field]: value } : t
      );
      // Fallback: If a translation doesn't exist yet, create it
      if (!updatedTranslations.some(t => t.language === langId)) {
        updatedTranslations.push({ language: langId, name: '', description: '', [field]: value });
      }
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const endpoint = isEdit ? `${BACKEND_URL}/api/included/${currentItem._id}` : `${BACKEND_URL}/api/included`;
    ...
    try {
      ...
      handleApiError(err, `Failed to ${isEdit ? 'update' : 'create'} included item.`);
    }
  }, [BACKEND_URL, authToken, isEdit, currentItem, fetchIncludedItems, onClose, handleApiError, validateForm]);
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <h3 className="text-lg sm:text-xl font-semibold text-havanaBlue mb-4">{isEdit ? 'Edit Included Item' : 'Add Included Item'}</h3>
                    ...
                    <div key={lang._id} className="border p-3 mb-2 rounded-md bg-gray-50">
                      ...
                      <div className="mt-2 grid grid-cols-1 gap-4">
                        <div>
                          ...
                          />
                          {formErrors[`translations[${languages.findIndex(l => l._id === lang._id)}].name`] && <p className="text-red-500 text-xs mt-1">{formErrors[`translations[${languages.findIndex(l => l._id === lang._id)}].name`]}</p>}
                        </div>
                        <div>
...
export default IncludedFormModal;